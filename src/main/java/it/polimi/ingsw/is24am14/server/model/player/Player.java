...
public class Player {
    ...
    public void removeCardFromHand(int index) throws IllegalArgumentException, IllegalStateException {
        if (index < 0) throw new IllegalArgumentException("The index is out of bounds.");
        if (this.playerHand.size() == 0) throw new IllegalStateException("The player's hand is empty, therefore it is not possible to remove a card from it.");
